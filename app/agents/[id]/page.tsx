  ...
  if (!agent) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <Link href="/discover?tab=agents">
          <Button variant="outline"><ArrowLeft className="size-4 mr-2" />{t("agents.backToMarket")}</Button>
        </Link>
      </div>
    );
  }
    ...
    try {
      ...
      if (data.steps) {
        ...
        setSteps([{
          id: "error",
          type: "response",
          content: `${t("agents.error")}: ${data.error}`,
          tokens: 0,
          durationMs: 0,
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Play className="size-3" />{agent.runs.toLocaleString()} {t("agents.runsCount")}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <Textarea
              ...
              placeholder={t("agents.enterRequest")}
              rows={4}
            ...
            {(steps.length > 0 || running) && (
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-pixel text-[9px] text-emerald-400 uppercase tracking-wider">{t("agents.execLog")}</span>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass-card rounded-xl p-5 border border-white/[0.06]">
            <h4 className="font-pixel text-[8px] text-muted-foreground uppercase tracking-widest mb-3">{t("agents.tools")}</h4>
            <div className="space-y-2">
              ...
              {toolsUsed.length === 0 && (
                <span className="text-xs text-muted-foreground">{t("agents.noTools")}</span>
              )}
            </div>
          </div>
          ...
          {/* Stats */}
          <div className="glass-card rounded-xl p-5 border border-white/[0.06]">
            <h4 className="font-pixel text-[8px] text-muted-foreground uppercase tracking-widest mb-3">{t("agents.stats")}</h4>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: t("agents.totalRuns"), value: agent.runs.toLocaleString() },
                { label: t("agents.successRate"), value: `${agent.successRate}%` },
                { label: t("agents.avgLatency"), value: `${(agent.avgLatencyMs / 1000).toFixed(1)}s` },
                { label: t("agents.forks"), value: String(agent.forks) },
              ].map((s) => (
