...
export function WorkflowsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ type: "success", text: `Workflow "${wfName}" saved!` });
    } catch {
  ...
  const handleTestRun = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setMessage({ type: "success", text: `Test complete: ${JSON.stringify(data).slice(0, 200)}` });
    } catch {
      setMessage({ type: "error", text: "Test run failed, please try again later" });
    }
  };

  return (
    <>
      {/* Sub-tabs - NES Buttons */}
      <div className="flex gap-2 mb-8 justify-center">
        {(["browse", "builder"] as const).map((t) => (
          ...
          >
            {t === "browse" ? t_("workflows.marketplace") : t_("workflows.create")}
          </button>
        ))}
      </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant="outline" className="shrink-0 border-violet-500/30 text-violet-300">
                    {wf.steps.length} {t_("workflows.steps")}
                  </Badge>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card-strong rounded-xl p-5 space-y-4">
            ...
            <textarea
              ...
              placeholder={t_("workflows.descOptional")}
              rows={2}
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center w-full max-w-lg">
                    ...
                    >
                      ...
                      {/* Condition input */}
                      <div className="mt-2">
                        <label className="font-pixel text-[7px] text-muted-foreground block mb-1">{t_("workflows.condition")}</label>
                        <input
                          value={step.condition}
                          onChange={(e) => updateCondition(step.id, e.target.value)}
                          placeholder={t_("workflows.conditionEmpty")}
      ...
      {message && (
        <div
          className="rpgui-container framed text-center"
      ...
      {showTestModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowTestModal(false)}>
          <div className="glass-card-strong rounded-xl p-6 w-full max-w-md mx-4 space-y-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold">{t_("workflows.testRun")}</h3>
            <textarea
              value={testInput}
